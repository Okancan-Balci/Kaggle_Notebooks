...
* Imputation is needed for `Age` and `Embarked` variables since they have missing values. Most tree models don't need imputation but I am going to use different families of models such as Neural Nets or Support Vector Machines. 
...
* `Name` variable has titles in it. Those titles would come handy. Further investigation is needed.
...
Observations which miss `Cabin` variable have lower survival rates and this could be used in the prediction process. However, I can't make sure that this pattern will be repeated in the test set so I am dropping the variable completely. 
...
Other variables look very useful so let's investigate them closely.
...
titanic %>% 
  ...
  geom_violin(aes(fill = Survived), show.legend = FALSE, color = NA)+
...
titanic %>% 
  ...
  geom_violin(width = 0.7, color = NA)+
...
* Survival Rate gets higher as fares increase. This might change if Pclass is taken into consideration.
...
* For Pclass 1 `Fare` correlates with survival. For Pclass 2 it still has some correlation but not as strong as Pclass1. However, because of the presence of very high values it's not really healthy to comment on this plot. We'll come back to this variable.
...
Now False Negatives. This time prediction is survived but the ground truth is not survived.
...
This time the model does the exact opposite. Females are mostly predicted for survival and for Pclass 1 and 2 this is a very solid prediction. However, this prediction fails for Pclass 3 females because they only had 50% survival rate. Again, I think we need more information about Class 3 in order to be able to predict better.

Let's now apply Down Sampling and see how results would differ.
...
As you can see two groups now have the same amount of observations.

(*) indicates increased Specificity.
...
## Model Evaluation and Some Takeaways

We improved the Specificity for almost all models. For the first ensemble it was 66% now it is 73%. This means that now the model can identify 11 more survivors. However, the Sensitivity took a big hit so does the Accuracy. A reason for that accuracy loss might be the decreased sample size because we down sampled and it cost us 207 observations. Considering the total sample, this is a serious information loss.

This new result isn't necessarily bad. There may be circumstances where the benefit of predicting 11 more True Negatives outweighs the cost of losing 21 True Positives. In the case of accuracy though, I couldn't make down sampling work. 

The other thing that I would like to talk is the performance of Nearest Neighbors algorithm. Strangely the model got better with down sampled data and its Specificity decreased! It's probably me being bad at ML but I wanted to report this. I want to note that parsimony usually worked better for kNN.

Lastly, I feel like given a sufficient amount of data, down sampling would be a good approach to balance Sensitivity and Specificity and potentially getting better accuracy results.

## All Models So Far

For the sake of curiosity let's band all models together and check the results.
...
I was hoping to get better results with up sampling but I couldn't. It feels like increasing the number of observations with replacement is diluting the data and because of this models can't really calculate the nuances. And the other reason is that I got bored playing with formulas so maybe I didn't iterate enough.

In this case Specificity increased for almost all models too but not as aggressively as it happened with Down Sampling.
...
Groups have equal amount of observations.
...
Now I am going to re-apply all pre-processing actions to the actual test set.
...
Random Forests and The Ensemble Models with the original train set had the best accuracy results so I am going to submit them both.
...
kagg_test %>% 
  select(PassengerId) %>% 
  mutate(Survived = predict(fit.rf.f, new_data = kagg_test) %>% 
           pull(.pred_class)) %>% 
  write_csv("Predictions_RF.csv")
```
...
```{r}
ensemble_preds <- tibble(
  logis = predict(fit.logis.f, new_data = kagg_test) %>% pull(.pred_class),
  xgb = predict(fit.xgb.f, new_data = kagg_test) %>% pull(.pred_class),
  rf = predict(fit.rf.f, new_data = kagg_test) %>% pull(.pred_class),
  nn = predict(fit.nn.f, new_data = kagg_test) %>% pull(.pred_class),
  svm = predict(fit.svm.f, new_data = kagg_test) %>% pull(.pred_class),
  knn = predict(fit.knn.f, new_data = kagg_test) %>% pull(.pred_class),
  nb = predict(fit.nb.f, new_data = kagg_test) %>% pull(.pred_class)
) %>% 
  mutate(across(everything(), as.character),
         across(everything(), as.numeric)) %>% 
  mutate(ensemble = rowSums(across(everything()))) %>% 
  mutate(preds = ifelse(ensemble > 3.5, 1, 0)) %>% 
  pull(preds)
...
kagg_test %>% 
  select(PassengerId) %>% 
  mutate(Survived = ensemble_preds)%>% 
  write_csv("Predictions_Ensemble.csv")
```

* Kaggle Score : 0.79665

With my test set both models had the same accuracy but Random Forests had better score on Kaggle.

# Some Closing Remarks

To be honest this project was a little bit longer than it should have been. However, playing around really helped me build a better intuition about these popular Machine Learning algorithms. 

The results can be be improved with hyper-parameter optimization and cross-validation. Furthermore, there may be better feature engineering practices. 
